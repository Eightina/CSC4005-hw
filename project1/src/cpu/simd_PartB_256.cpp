...
int main(int argc, char** argv) {
    ...
    for (int height = 1; height < input_jpeg.height - 1; height++) {
        for (int width = 1; width < input_jpeg.width - 1; width++) {
            ...
            // solve red
            auto temp_red = new unsigned char[32];
            ...
            __m256 row2_filtered = _mm256_mul_ps(row2_floats, filter2);
            ...
            __m256i add_results_red_8 = _mm256_shuffle_epi8(add_results_red_32, shuffle);
            _mm256_storeu_si256((__m256i*)(&temp_red), add_results_red_8);
            char final_red = temp_red[0] + temp_red[1] + temp_red[2];
            // char red_res = _mm256();
            // https://www.jianshu.com/p/523f262c77b3
            // solve green
            // row0 = _mm_loadu_si128((__m128i*) (greens+start0));
            // row0_ints = _mm256_cvtepu8_epi32(row0);
            // row0_floats = _mm256_cvtepi32_ps(row0_ints);
            // row0_filtered = _mm256_mul_ps(row0_floats, filter0);

            // row1 = _mm_loadu_si128((__m128i*) (greens+start1));
            // row1_ints = _mm256_cvtepu8_epi32(row1);
            // row1_floats = _mm256_cvtepi32_ps(row1_ints);
            // row1_filtered = _mm256_mul_ps(row1_floats, filter1);

            // row2 = _mm_loadu_si128((__m128i*) (greens+start2));
            // row2_ints = _mm256_cvtepu8_epi32(row2);
            // row2_floats = _mm256_cvtepi32_ps(row2_ints);
            // row2_filtered = _mm256_mul_ps(row2_floats, filter2);
